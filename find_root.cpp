...
optional<double> find_root(int dim, double *alpha, double *lam, double rho, double rho_tol) {
    ...
    nu_min = (ln > 0) ? 0.0 : (ln < 0) ? -ln : 0.0;
    nu_a = (ln > 0) ? 0.0 : (ln < 0) ? nu_min : 1.0;
    nu_b = (ln > 0) ? ln : (ln < 0) ? nu_min * 2.0 : 2.0;

    double f_a = calculate_length(dim, alpha, lam, rho, nu_min + nu_a);
    double f_b = calculate_length(dim, alpha, lam, rho, nu_min + nu_b);
...
optional<double> rec_find_root(int dim, double *alpha, double *lam, double rho, double rho_tol,
    ...
    // cout << ki << "\t" << nu_a << "\t" << nu_b << "\t" << f_a << "\t" << f_b << endl;

    if (ki >= MAX_ITERATIONS) return optional<double>((nu_a + nu_b) / 2.0);
    else if (isinf(f_a) || isinf(f_b)) return nullopt;
    else if (f_a >= 0 && f_b <= 0) {
        ...
        f_new = calculate_length(dim, alpha, lam, rho, nu_min + nu_new);
